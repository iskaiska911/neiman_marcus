...
def get_all_categories():
    response = SCRAPFLY.scrape(ScrapeConfig(url='https://www.neimanmarcus.com/en-kz'))
...
def get_all_subcategories(catgegory):
    response = SCRAPFLY.scrape(ScrapeConfig(url=f'https://www.neimanmarcus.com/en-kz/c{catgegory}'))
    assert response.status_code == 200
...
async def scrape_slugs(url: str) -> list:
    ...
    for i in range(1, page + 1):
        ...
        result = await SCRAPFLY.async_scrape(ScrapeConfig(modified_url, **BASE_CONFIG,retry=True))
...
async def scrape_product(url: str) -> Dict:
    ...
    time.sleep(1)
    try:
        result = await SCRAPFLY.async_scrape(ScrapeConfig(url, **BASE_CONFIG,render_js=True,rendering_wait=5000,retry=True))
